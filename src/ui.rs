...
pub fn ui(
                    ...
                    ui.push_id(index, |ui| {
                        ui.collapsing(layer.layer_type.to_string(), |ui| {
                            ...
                            if layer_type_value != layer.layer_type.to_string() {
                                layer.layer_type =
                                    LayerType::from_str(layer_type_value.as_str()).unwrap();
                                layer.settings =
                                    LayerSettings::from_layer_type(layer.layer_type.clone());
    ...
    let pos = UVec2::new(
        (panel.response.rect.width() * window.scale_factor()) as u32,
        0,
    );
